...
# PS
# ---
    my $LocalParamObject = $Kernel::OM->Get('Kernel::System::Web::Request');
    $Self->{Dashboard}   = $LocalParamObject->GetParam( Param => 'Dashboard') // '';

    if ( $Self->{Dashboard} ) {
        $UserSettingsKey .= $Self->{Dashboard};
        $Param{DashboardIndex} = $Self->{Dashboard};
    }
# ---
...
# PS
# ---
            my $ExtraParams = $Self->{Dashboard} ? '&Dashboard=' . $Self->{Dashboard} : '';
# ---
...
#                OP => "Action=$Self->{Action}",
                OP => "Action=$Self->{Action}$ExtraParams",
# ---
            );
        }
